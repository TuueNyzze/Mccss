...
## `sovereign-gate/` — unrelated subsystem

`sovereign-gate/` is a standalone DSL compiler/verifier prototype for a `MONOLITH.DELTA` spec (a one-way, hardware-enforced gate design for a dual-RP2040 Executor/Sentry topology). It has nothing to do with the MCCSS substrate or Edenfield admin app, has its own `package.json`/tests, and is not wired into the Docker Compose demo or root CI. See `sovereign-gate/README.md` for details, usage (`npm test`, `npm run compile`), and design notes from reviewing the spec.
